...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # 4. Tasks (V6 Update: Creator User, Visibility, Assignments)
        c.execute('''CREATE TABLE IF NOT EXISTS tasks (
            ...
            assignee_ids TEXT, -- JSON List of user_ids
...
class TaskCreate(BaseModel):
    user_id: str
    org_id: Optional[int] = None # Optional now (Public tasks by individuals)
    title: str
    description: str
    xp_reward: int
...
# --- Helpers ---
def check_org_task_permission(user_id: str, org_id: int, conn):
    """Check if user can create tasks for an Org."""
    c = conn.cursor()
    c.execute("SELECT owner_user_id FROM organizations WHERE org_id=?", (org_id,))
    org = c.fetchone()
    if org and org[0] == user_id: return True
...
def update_org(update: OrgUpdate):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if update.name:
            try:
                c.execute("UPDATE organizations SET name=? WHERE org_id=?", (update.name, update.org_id))
            except sqlite3.IntegrityError:
                raise HTTPException(status_code=400, detail="Name taken")
        conn.commit()
...
def create_task(task: TaskCreate):
    # Logic:
    # 1. If PRIVATE: Must have org_id AND check_org_task_permission
    # 2. If PUBLIC:
    #    - If org_id provided: check_org_task_permission (Org-sponsored Public Task)
    #    - If no org_id: Allow (Individual Public Task)
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if task.visibility == 'PRIVATE':
            ...
            if not check_org_task_permission(task.user_id, task.org_id, conn):
        ...
        assignees_json = json.dumps(task.assignee_ids or [])
        
        c.execute('''INSERT INTO tasks (creator_org_id, creator_user_id, title, description, xp_reward, difficulty, deadline, status, visibility, assignee_ids) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, 'OPEN', ?, ?)''', 
                  (task.org_id, task.user_id, task.title, task.description, task.xp_reward, task.difficulty, task.deadline, task.visibility, assignees_json))
        conn.commit()
    return {"status": "success"}

@app.get("/tasks/available", tags=["Tasks"])
def avail_tasks(user_id: Optional[str] = None):
    # Logic:
    # Return ALL PUBLIC tasks.
    # Return PRIVATE tasks where user is assignee OR member of creator_org.
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        member_org_id = None
        if user_id:
            c.execute("SELECT member_org_id FROM users WHERE user_id=?", (user_id,))
            row = c.fetchone()
            if row: member_org_id = row[0]
        
        tasks = []
        c.execute('''SELECT t.*, o.name as org_name, u.username as creator_name
                     FROM tasks t 
                     LEFT JOIN organizations o ON t.creator_org_id=o.org_id
...
def create_role(role: RoleCreate):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        org = c.fetchone()
        if not org or org[0] != role.owner_user_id: raise HTTPException(status_code=403, detail="Owner only")
...
def assign_role(assign: RoleAssign):
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        c.execute("UPDATE users SET org_role_id = ? WHERE user_id = ?", (assign.role_id, assign.target_user_id))
        conn.commit()
    return {"status": "success"}

@app.post("/org/transfer-ownership", tags=["Advanced Org"])
def transfer(transfer: TransferOwnership):
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        if not verify_password(transfer.current_owner_id, transfer.password, conn): raise HTTPException(status_code=401)
        c.execute("UPDATE organizations SET owner_user_id = ? WHERE org_id = ?", (transfer.new_owner_id, transfer.org_id))
        conn.commit()
    return {"status": "success"}
...
def get_stats(org_id: int):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        active = c.fetchone()[0]
        c.execute('''SELECT COUNT(*) FROM submissions s JOIN tasks t ON s.task_id=t.task_id WHERE t.creator_org_id=? AND s.status='PENDING' ''', (org_id,))
...
def get_reviews(org_id: int):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute('''SELECT s.submission_id, s.proof_link, t.title as task_title, u.username as student_name, t.xp_reward 
                     FROM submissions s JOIN tasks t ON s.task_id=t.task_id JOIN users u ON s.user_id=u.user_id 
...
def review(review: ReviewAction):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        c.execute("UPDATE submissions SET status=?, feedback=? WHERE submission_id=?", (status, review.feedback, review.submission_id))
        if status == 'APPROVED':
...
def submit(sub: SubmissionCreate):
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        c.execute("INSERT INTO submissions (task_id, user_id, proof_link) VALUES (?,?,?)", (sub.task_id, sub.user_id, sub.proof_link))
        conn.commit()
    return {"status": "success"}
...
def profile(user_id: str):
    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute('''SELECT u.*, r.name as role_name FROM users u LEFT JOIN org_roles r ON u.org_role_id = r.role_id WHERE user_id=?''', (user_id,))
        user = c.fetchone()
        level = 1 + (user['total_xp'] // 100)
        c.execute("SELECT COUNT(*) FROM users WHERE total_xp > ?", (user['total_xp'],))
        rank = c.fetchone()[0] + 1
        c.execute("SELECT t.title, s.status FROM submissions s JOIN tasks t ON s.task_id=t.task_id WHERE s.user_id=?", (user_id,))
        hist = c.fetchall()
        return {"username": user['username'], "total_xp": user['total_xp'], "level": level, "rank": rank, "role_name": user['role_name'], "history": hist}

@app.get("/leaderboard")
def lb():
    with sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        c = conn.execute("SELECT username, total_xp FROM users ORDER BY total_xp DESC LIMIT 10")
        return c.fetchall()
